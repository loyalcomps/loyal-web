...
(select m.partner_id,sum(m.total_hours) as total_hours from(select distinct s.id, s.partner_id,(sl.product_uom_qty) as total_hours from account_analytic_line as an 
	left join project_project as p on p.id= an.project_id
 	full join sale_order as st on st.project_id=an.project_id
	full join sale_order as s on (s.id=p.sale_order_id)
	left join sale_order_line as sl on s.id=sl.order_id )as m
